...
-- 별칭에 띄어쓰기나 특수기호가 없다면 홑따음표(')와 AS는 생략 가능하다.
SELECT
      MENU_CODE AS 'CODE',
      MENU_NAME NAME,
      MENU_PRICE
...
/* SELF JOIN */
-- 같은 테이블 내에서 행과 행 사이의 관게를 찾기위해 사용되는 QSL JOIN
...
SELECT
  ...
  -- MYSQL은 기본적으로 NEWTED LOOP JOIN을 사용하지만 이를 강제하려면 HASH JOIN을 사용하지 않도록 강제화 할 수 있다.
SELECT
      A.MENU_NAME,
      B.CATEGORY_NAME
  FROM TBL_MENU A
  JOIN TBL_CATEGORY B ON A.CATEGORY_CODE = B.CATEGORY;
  
  
/* HASH JOIN */
-- HASH JOIN은 등가 조언('=' 연산자를 사용하는 조인)에만 사용할 수 있고 비등가 조인에는 사용할 수 없다.
SELECT
       A.MENU_NAME,
       B.CATEGORY_NAME
   FROM TBL_MENU A
